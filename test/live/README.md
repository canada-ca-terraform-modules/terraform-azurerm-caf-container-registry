...
this module's `.github/workflows/live-test.yml`) to prove that an open PR
doesn't destroy or replace a resource a real consumer already has running.
It is **not** a substitute for either of the module's other two test
surfaces:
...
| `variables.tf` | `env`, `location` (defaults to `canadacentral`), `tags`, `pr_number` (defaults to `"manual"`), and `container_registry` (typed `any`, passed straight through to the module). |
| `config/container_registry.tfvars` | One representative real-usage fixture: a plain Premium registry, admin disabled, public network access disabled, default AcrPull user-assigned identity, no `for_each` fan-out. |
...
cd test/live
terraform init
terraform plan  -var-file=config/container_registry.tfvars
terraform apply -var-file=config/container_registry.tfvars
```

Confirm only the live-test resource group and `module.container_registry`
are planned/applied, then tear it down:

```bash
terraform destroy -var-file=config/container_registry.tfvars
```
...
terraform apply -var-file=config/container_registry.tfvars -var="pr_number=<pr-number>"

# 2. PR plan (and, in CI, apply), from A, against the same state file.
cd A/test/live
terraform init -backend-config="path=$STATE"
terraform plan -var-file=config/container_registry.tfvars -var="pr_number=<pr-number>"

# 3. Always tear down from A once the run finishes (`if: always()` in CI).
terraform destroy -var-file=config/container_registry.tfvars -var="pr_number=<pr-number>"
```
